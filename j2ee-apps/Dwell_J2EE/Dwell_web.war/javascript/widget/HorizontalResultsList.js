...
dojo.declare(
	...
	[dijit._Widget, dijit._Templated, dijit._Container],
	{
		...
		templatePath: dojo.moduleUrl("atg.store.widget", "template/horizontalResultsList.txt"),
		templateString: "",
		xhr: "",
		data: null,
		standby: null,
		...
		startup: function() {
			this.itemIdPrefix = this.id + "-item";
		...
		addIdsToListItems: function(){
			...
			var self = this;
      		...
      		dojo.query("ul > li", this.containerNode).forEach(function(node, index, array){
          		dojo.attr(node, "id", self.itemIdPrefix + counter++);
      		});
		},
		...
		changeProductsInView: function(changeNumber){      
			...
			dojox.fx.smoothScroll({
				node: dojo.byId(this.itemIdPrefix + this.firstProductInViewNumber),
