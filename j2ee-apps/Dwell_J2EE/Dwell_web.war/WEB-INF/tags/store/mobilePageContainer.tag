<%--
  ...
  It introduces the following request-scoped variables:
    ...
    mobileImagesPrefix
      Base URL of the CRS-M images (/crsdocroot/content/mobile/images/).
    storeName
      Currently selected ATG store name.
  ...
  NOTES:
    1) The "endecaUserAgent" request-scoped variable (request attribute), which is used by this tag,
      is set in the "MobileDetectionInterceptor" Nucleus component: it's set, when the "Endeca Preview" is enabled
    ...
    2) The "rootContentItem" request-scoped variable (request attribute), which is used by this tag,
...
<c:set var="mobileImagesPrefix" value="/crsdocroot/content/mobile/images/" scope="request"/>
...
<html lang="${language}">
  <head>
    ...
    <link rel="apple-touch-icon" href="${mobileImagesPrefix}apple-touch-icon.png"/>
    ...
    <title><c:out value="${storeName}"/> <fmt:message key="mobile.common.hyphen"/> <c:out value="${titleString}"/></title>
