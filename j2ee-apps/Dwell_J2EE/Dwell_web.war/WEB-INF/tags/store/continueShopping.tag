<%--
  ...
  or to '/home' if this doesn't exist or is not valid for the users current context.
  ...
  Example:
    <crs:continueShopping>
      <c:set var="continueShoppingLink" value="/home"/>
...
<%-- 
  The /OriginatingRequest.contextPath returns "" when a servlet is in the 
  default (root) context. When the context root is "", we must set it to "/" in order
  for JSTL url tags that define the 'context' attribute to function correctly.
--%>
...
<%--
  Generate the link to the category page or set the continue shopping link to '/home'.
--%>
<c:choose>
    ...
    <dsp:droplet name="CategoryLookup">
      ...
      <dsp:oparam name="output">
        ...
        <c:choose>
          ...
          <c:otherwise>
             <c:set var="continueShoppingURL" value="${contextroot}/home"/>
          </c:otherwise>
      ...
      <dsp:oparam name="wrongCatalog">
        ...
        <c:url var="continueShoppingURL" context="${contextroot}" value="/home"/>
      </dsp:oparam>
