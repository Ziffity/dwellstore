...
<dsp:page>
  ...
  <dsp:getvalueof var="rootContentItem" param="rootContentItem"/>
  ...
  <dsp:droplet name="BrowserTyperDroplet" var="currentBrowser">
    <dsp:oparam name="output">
      ...
      <c:if test="${currentBrowser.browserType eq 'robot'}">
        ...
        <c:if test="${not empty categoryId 
                        || not empty productId
                        || not empty rootContentItem}">

          <if test="${not empty rootContentItem}">   
            <%--
              ...
              Output parameters:    
                actionURL
                  An URL for the passed in action Object.
           
              Open parameters: 
                output
                  The output oparam is rendered when the not empty URL 
                  is represented by the Action object.   
            --%>
            <dsp:droplet name="ActionURLDroplet">
              <dsp:param name="action" value="${rootContentItem.canonicalLink}"/>
              
              <dsp:oparam name="output">
                <dsp:getvalueof var="actionURL" param="actionURL"/>
                <c:set var="canonicalLink" value="${actionURL}"/>
              </dsp:oparam>
            </dsp:droplet>
          </if>
          ...
          <%-- 
            ...
            Input parameters
              ...
              canonicalLink
                The canonical link returned in the rootContentItem. This
                may be null, for example, if we are on a non-Endeca driven page.
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/global/gadgets/documentLinks.jsp#1 $$Change: 946917 $--%>