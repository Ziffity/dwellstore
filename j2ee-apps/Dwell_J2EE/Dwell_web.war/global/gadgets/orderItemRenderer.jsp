<%--
  ...
  It displays item's site, product icon, SKU info, quantity, price and applied 
  discounts.
...
<dsp:page>
  ...
  <dsp:getvalueof var="hideSiteIndicator" vartype="java.lang.String" 
                  param="hideSiteIndicator"/>
  ...
  <dsp:getvalueof var="currentItem" vartype="atg.commerce.order.CommerceItem" 
                  param="currentItem"/>
  <dsp:getvalueof var="commerceItemRelationship" 
                  param="commerceItemRelationship"/>
  <dsp:getvalueof param="currentItem.auxiliaryData.productRef.NavigableProducts" 
                  var="navigable" vartype="java.lang.Boolean"/>
  <dsp:getvalueof var="displayProductAsLink" vartype="java.lang.Boolean" 
                  param="displayProductAsLink"/>
  ...
  <c:choose>
    <c:when test="${not empty commerceItemRelationship}">
      ...
      <%--
        Generate price beans for each product row on page. We will 
        use them to display all applied discounts per cart item.
        ...
        Output Parameters:
          priceBeansMap
            A map whose key is the commerce item id and whose 
            value is a price bean.
      --%> 
      <dsp:droplet name="/atg/store/droplet/StorePriceBeansDroplet">
        ...
        <dsp:oparam name="output">
          ...
          <dsp:getvalueof var="gwpPriceBeansQuantity" 
                          param="gwpPriceBeansQuantity" />
        </dsp:oparam>
      </dsp:droplet>
    </c:when>
    <c:otherwise>
      <%--
      Generate price beans for each product row on page. We will use 
      them to display all applied discounts per cart item.
      ...
      Output Parameters:
        priceBeansMap
          A map whose key is the commerce item id and whose value 
          is a price bean.
      --%> 
      <dsp:droplet name="/atg/store/droplet/StorePriceBeansDroplet">
        <dsp:param name="order" param="order"/>
  ...
  <dsp:getvalueof var="nonGWPQuantity" 
                  value="${priceBeansQuantity - gwpPriceBeansQuantity }"/>
    <%-- 
      Only display the non-GWP item when its quantity is greater than 
      the GWP quantity, as GWP items will be displayed in separate
      rows. Also display an existing non-GWP item when the price beans
      and GWP price beans quantities are 0. This indicates that the order 
      hasn't been placed by the user yet.
    --%>
    <c:if test="${(nonGWPQuantity > 0) 
                    or (not empty commerceItemRelationship 
                          and priceBeansQuantity == 0 
                          and gwpPriceBeansQuantity == 0)}">
      <tr>
        <%-- Display site indicator, if needed. --%>
        <c:if test="${empty hideSiteIndicator 
                        or (hideSiteIndicator == 'false')}">
          <td class="site">
            <dsp:getvalueof var="displaySiteIcon" vartype="java.lang.Boolean" 
                            param="displaySiteIcon"/>
            <c:if test="${displaySiteIcon or empty displaySiteIcon}">
              <dsp:include page="/global/gadgets/siteIndicator.jsp">
                ...
                <dsp:param name="siteId" 
                           param="currentItem.auxiliaryData.siteId"/>
                <dsp:param name="product" 
                           param="currentItem.auxiliaryData.productRef"/>
              </dsp:include>
            </c:if>
          </td>
        </c:if>
        ...
        <td class="image">
          <c:choose>
            <c:when test="${missingProductId 
                != currentItem.auxiliaryData.productRef.repositoryId 
                && navigable && displayProductAsLink}">
        ...
        <td class="item" scope="row" abbr="${productDisplayName}">
          <span class="itemName">              
            <%-- 
              Display product name as link, if proper template is 
              defined for the current product. 
            --%>
            ...
            <c:choose>
              <c:when test="${empty pageurl || !navigable}">
                <%-- 
                  Either we have no proper URL, or product is not navigable. 
                  Do not display link. 
                --%>
              ...
              <c:otherwise>
                <c:choose>
                  <c:when test="${(missingProductId 
                    != currentItem.auxiliaryData.productRef.repositoryId 
                    or empty missingProductId) and displayProductAsLink}">
          ...
          <dsp:include page="/global/util/displaySkuProperties.jsp">
            <dsp:param name="product" 
                       param="currentItem.auxiliaryData.productRef"/>
            <dsp:param name="sku" 
                       param="currentItem.auxiliaryData.catalogRef"/>
        ...
        <%-- 
          Include item's quantity and detailed price (including discounts 
          applied). 
        --%>
        ...
        <td class="total">
            <%--
              If there are price beans calculated, display their amount, 
              not commerce item's (SG-CI relationship case). This gadget is 
              being used for displaying commerce item's details in two cases.
              First is single shipping group case. In this case one can take 
              total and quantity from commerce item itself, because commerce 
              item is not being splitted into several rows. Second is multiple 
              shipping groups case. In this case different rows can have 
              different discounts applied to their items (in case of item-level 
              discounts applied). Hence we have to display quantity, total and 
              discounts from price beans generated for this row (or 
              relationship).
            --%>
          <dsp:getvalueof var="amount" vartype="java.lang.Double" 
                          value="${priceBeansAmount}"/>
          <c:if test="${empty amount}">
            <dsp:getvalueof var="amount" vartype="java.lang.Double" 
                            param="currentItem.priceInfo.amount" />
          </c:if>
          <p class="price">
        ...
        <c:if test="${priceBean.giftWithPurchase}">
          <c:forEach begin="1" end="${priceBean.quantity}">
            <dsp:include page="/global/gadgets/orderGiftItem.jsp">
              ...
              <dsp:param name="displayProductAsLink" 
                         param="displayProductAsLink"/>
              <dsp:param name="displayAvailabilityMessage" 
                         param="displayAvailabilityMessage"/>
          </dsp:include>                        
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/global/gadgets/orderItemRenderer.jsp#1 $$Change: 946917 $--%>