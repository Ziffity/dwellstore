<%--
  Page renders related products for the currently viewed product or article. The relatedProducts 
  are filtered based on the cart sharing site group to which the current site belongs.
  ...
  Required Parameters:
    ...
    article
      the article repository item for which related products should be displayed.
...
<dsp:page>
  ...
  <dsp:getvalueof var="article" param="article" />
  <c:if test="${not empty product}">
    <dsp:getvalueof var="relatedProducts" value="${product.relatedProducts}"/>
  </c:if>
  <c:if test="${not empty article}">
    <dsp:getvalueof var="relatedProducts" value="${article.relatedProducts}"/>
  </c:if>
    
  <%-- Check that related products is not empty. --%>
  <c:if test="${not empty relatedProducts}">
    ...
    <dsp:droplet name="ItemSiteGroupFilterDroplet">
      <dsp:param name="collection" value="${relatedProducts}"/>
        ...
        <div id="atg_store_recommendedProducts" class="atg_store_relatedProducts">
          <h3>
            <c:choose>
              ...
              <c:otherwise>
                <fmt:message key="article_details.relatedProducts" />
              </c:otherwise>
            </c:choose>            
          </h3>
          <ul>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/browse/gadgets/relatedProducts.jsp#2 $$Change: 953229 $--%>
